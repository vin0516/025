...
public partial class Book_Sample_Ch03_WebControls_CheckBoxList_DB2 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try     //==== 以下程式，只放「執行期間」的指令！=====================
        {
            ...
            while (dr.Read())
            {
                ListItem myListItem = new ListItem(dr["TheText"].ToString(), dr["TheValue"].ToString());

                if (dr["TheFlag"].ToString() == "y")
                {
                    myListItem.Selected = true;
                }
                CheckBoxList1.Items.Add(myListItem);
            }
            //*******************************************************
