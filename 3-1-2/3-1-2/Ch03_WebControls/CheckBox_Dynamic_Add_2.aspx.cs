...
public partial class Book_Sample_Ch03_WebControls_CheckBox_Dynamic_Add : System.Web.UI.Page
{
    ...
    {
        //****** 重複的程式 縮減 成一個副程式 ******
        DynaAddCheckBox("CheckBox1", "第一層");
        DynaAddCheckBox("CheckBox2", "第二層");
        DynaAddCheckBox("CheckBox3", "第三層");
    ...
    //**** 自己寫的副程式 ****
    protected void DynaAddCheckBox(string myID, string myText)
    {
        CheckBox CB = new CheckBox();
              CB.ID = myID;    //CB.ID = "CheckBox1";
              CB.AutoPostBack = true;
              CB.Text = myText;    //CB.Text = "第一層";
              CB.CheckedChanged += new System.EventHandler(this.myCB_CheckedChanged);  // 動態加入事件，後面是「事件名稱」
